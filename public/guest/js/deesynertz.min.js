        ...
        $.ajax({
            ...
            success: function (feedback) {
                ...
                if (feedback.result === "sent") {
                    ...
                    $('#send_otp').html("<i class='fa fa-check'></i> Code sent to 255******"+feedback.last3digits+"</span>").attr(
                        'disabled', 'disabled')
                    $('.register-form input, .register-form select').attr('readonly', 'readonly');
                    $('#otp_codes').removeAttr('readonly');
                                        ...
                                        if (unit_id.length > 0) {
                                            ...
                                            setTimeout(function () {
                                                ...
                                                if(unit.length == 0) {
                                                    ...
                                                    window.open(getHouseDetailsURI+"/"+unit+"/"+prices+"/"+roomss+"/"+ward, '_SELF');
                                                }
                                            }, 3000);
                                        }
                                    }, 3000);
...
function notchON(btnclass, text = '') {
    $('.' + btnclass).html("<span class='fa fa-circle-notch fa-spin'></span> " + text);
}

function notchDONE(btnclass, text = 'Done') {
    $('.' + btnclass).html("<i class='fa fa-check'></i> "+text+"</span>").attr('disabled','disabled');
}
...
function sharetoearn() {
    ...
    $('#generate-link-btn').on('click', function(){
        $('#roleId').val('Dalali')
        // $(this).hide();
        notchON('generate-link-btn', 'Generating')
        ...
        $.post(generateLinkURL, {
            ...
            notchDONE('generate-link-btn')
            window.location.reload();
            // $('#link-loader, .spinner').hide();
            // $("#link").text(feedback.url);
            // $('.link-item').fadeIn();
        });
    });
}
        ...
        $.post(likeurl,
            {
                ...
                if (feedback.status === 'liked') {
                    ...
                    $('#like-txt').text('Dislike')
                } else if (feedback.status === 'unliked') {
                    $('#like-heart').removeClass('text-danger')
                    $('#like-txt').text('Like')
                }
                $('#total-likes').text(feedback.likes)
            })
    })
}
